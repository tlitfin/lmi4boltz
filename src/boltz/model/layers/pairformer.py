...
class PairformerLayer(nn.Module):
    ...
    def forward(
        ...
        z += self.tri_mul_out(z, mask=pair_mask, triangle_mult_gate_nchunks=triangle_mult_gate_nchunks, use_kernels=use_cuequiv_mul or use_kernels)

        #dropout = get_dropout_mask(self.dropout, z, self.training)
        #z = z + dropout * self.tri_mul_in(
        get_dropout_mask(self.dropout, z, self.training)
        z += self.tri_mul_in(z, mask=pair_mask, triangle_mult_gate_nchunks=triangle_mult_gate_nchunks, use_kernels=use_cuequiv_mul or use_kernels)
